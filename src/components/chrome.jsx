//this is a modified & slighlty improved version of this source https://github.com/uiwjs/react-color/tree/main/packages/color-chrome
...
const Chrome = React.forwardRef((props, ref) => {
	...
	const handleWheelOpacity = (e) => {
		//Handle opacity on scroll
		if (e.deltaY < 0) {
			if (hsva.a < 1) {
				handleChange({ ...hsva, a: hsva.a + 0.01 });
			}
		} else {
			if (hsva.a > 0) {
				handleChange({ ...hsva, a: hsva.a - 0.01 });
  ...
  const handleWheelHue = (e) => {
		//Handle hue on scroll
		if (e.deltaY < 0) {
			if (hsva.h < 360) {
				handleChange({ ...hsva, h: hsva.h + 1 });
			}
		} else {
			if (hsva.h > 0) {
				handleChange({ ...hsva, h: hsva.h - 1 });
